...
def format_results(result: dict):   # вывод результатов
    ...
    if result.get("status") in ["uploaded", "scanning"]:
        ...
        print("Чтобы получить результат, повторите проверку")
        return
    ...
    if result.get("type") == "file" and result.get("malicious_details"):
        print("\nДетали угроз:")
    ...
    if permalink:
        print(f"\nСсылка на полный отчет: {permalink}")
    else:
        print("\nСсылка на отчет недоступна")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if len(file_hash) != 64:
                print("Ошибка: Хеш SHA-256 должен содержать ровно 64 символа")
                continue
